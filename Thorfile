...
class TestRunner < Thor
  ...
  def error_checks
    puts "Attempting to update #{updated_files}"
    ...
    if file_types.keys.include?(:other)
      raise "A file you are trying to change is not a base file, cookbook json file, or a stencil"
    end

    # Raise an error if you are changing a file, but it does not
    # belong to a stencil.
    if stencil_map.values.include?(:no_stencils_found)
      raise "You're trying to change a file in a stencil set, but it does not belong to a stencil"
    end
    ...
    if !file_types[:stencil].empty? && updated_cookbooks.empty?
      raise "You are editing a stencil but there is no corresponding cookbook json file in cookbooks/"
    end
  end
